...
public class UserServiceImpl implements UserService {
    ...
    public UserView update(Integer userId, UserForm form) throws NotFoundException {
	 ...
	 return	userRepository.findById(userId).map((userObj) -> {
                    return new UserView(userRepository.save(userObj.update(form.getName(),
                            form.getEmail(),
                            passwordEncoder.encode(form.getPassword()))));
	 ...
	 @Override
	    public UserView update(UserForm form) {
	        UserView user = currentUser();
	        return update(user.getUserId(),form);
